...
public class QRcodeScanner extends AppCompatActivity {
    ...
    private final String TAG = QRcodeScanner.class.getSimpleName();
    ...
    public class MyImageAnalyer implements ImageAnalysis.Analyzer{
        ...
        public void analyze(@NonNull ImageProxy image) {
            Log.d(TAG, "MyImageAnalyer.analyze");
            mImageProxy = image;
            TransImageProxyToInputImage(image);
        }

        public void analyze(InputImage image) {
            scanBarCodeInputImage(image);
        }

        private void TransImageProxyToInputImage(ImageProxy image) {
            Log.d(TAG, "MyImageAnalyer.scanBarCode");
            @SuppressLint("UnsafeOptInUsageError") Image image1 = image.getImage();
            assert image1 != null;
            InputImage inputImage = InputImage.fromMediaImage(image1,
                    image.getImageInfo().getRotationDegrees());
//            Log.d(TAG, "scanBarCode Photo From Camera Format:"
//                    + inputImage.getFormat()
//                    + " Height" + inputImage.getHeight()
//                    + " width" + inputImage.getWidth());
            scanBarCodeInputImage(inputImage);         //<<================
        }

        private void scanBarCodeInputImage(InputImage inputImage) {
//            Log.d(TAG, "MyImageAnalyer.scanBarCodeInputImage");
//            Log.d(TAG, "scanBarCode Photo From Camera Format:" + inputImage.getFormat()
...
//                            Log.d("QRcodeScanner.ScanBarCode", "QRCODE successfully read");
//                            Log.d("QRcodeScanner.ScanBarCode", "QRCODE size: " + barcodes.size());
                            if(!barcodes.isEmpty()){
                                Log.d("QRcodeScanner.ScanBarCode", "QRCODE not empty");
//                                Bitmap bitmap = yuv420ToBitmap(inputImage);
//                                if (bitmap != null) {
//                                    Log.d("QRcodeScanner.ScanBarCode", "Bitmap is not null");
//                                    saveBitmapToGallery(bitmap, "Camera_QR_Code_Image");
//                                }else {
//                                    Log.e("QRcodeScanner.ScanBarCode", "Failed to convert InputImage to Bitmap.");
//                                }
    ...
    @OptIn(markerClass = ExperimentalGetImage.class)
    private Bitmap yuv420ToBitmap(InputImage imageProxy) {
        Log.d("QRcodeScanner.ScanBarCode", "get Image" +imageProxy.getFormat());
        Image image = imageProxy.getMediaImage();                  //Crush at Here   <<==============================
        Log.d("QRcodeScanner.ScanBarCode", "SaveImage.yuv420ToBitmap.Format:" +image.getFormat());
        if (image == null) {
            Log.e("QRcodeScanner.ScanBarCode", "image is null");
            return null;
        }
        Log.d("QRcodeScanner.ScanBarCode", "image is not null");
    ...
    private void saveBitmapToGallery(Bitmap bitmap, String title) {
        ...
        if (directory != null) {
            String fileName =  "SaveImageCameraCapture1111.jpg";
            File file = new File(directory, fileName);
            try (FileOutputStream out = new FileOutputStream(file)) {
                bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
                Log.d("QRcodeScanner.ScanBarCode", "Image saved: " + file.getAbsolutePath());

            } catch (IOException e) {
                Log.e("QRcodeScanner.ScanBarCode", "Failed to save image", e);
            }
        }
    }
    // ============================================================================ //
